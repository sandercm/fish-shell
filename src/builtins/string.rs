        ...
        while let Some(c) = w.next_opt() {
            match c {
                ':' => {
                    ...
                    return Err(Some(STATUS_INVALID_ARGS));
                }
                '?' => {
...
pub fn string(parser: &Parser, streams: &mut IoStreams, args: &mut [&wstr]) -> c_int {
    ...
    let result = match subcmd_name.to_string().as_str() {
